...
const APP_ID = "identity_platform.identity_platform_admin_ui"
...
func WithRequest(request *http.Request) Option {
	return func() []Field {
		return []Field{
			...
			zap.String("host_ip", request.Host),
			zap.String("hostname", request.URL.Hostname()),
			zap.String("protocol", request.Proto),
			zap.String("port", request.URL.Port()),
