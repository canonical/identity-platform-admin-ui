...
func TestCreateSchemaSuccessIfIDIsMissing(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	mockConfigMapV1.EXPECT().Update(gomock.Any(), cm, gomock.Any()).Times(1).DoAndReturn(
		func(ctx context.Context, configMap *v1.ConfigMap, opts metaV1.UpdateOptions) (*v1.ConfigMap, error) {
			...
			rawSchema, ok := configMap.Data[*c.Id]
			...
			if *c.Id == "" {
				t.Fatalf("expected ID to have defaulted to a uuid, not %v", i.Id)
			}
	...
	if *is.IdentitySchemas[0].Id == "" {
		t.Fatalf("expected schema ID to have defaulted to a uuid, not %v", is.IdentitySchemas[0].Id)
	}
